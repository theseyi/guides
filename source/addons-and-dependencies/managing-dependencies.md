...
You should only try to import assets located in the `vendor` folders. `bower_components` also still
...
```ember-cli-build.js
app.import('vendor/showdown/dist/showdown.js', {
  using: [
...
if (app.env === 'test') {
  ...
  app.import('vendor/sinonjs/sinon.js', { type: 'test' });
  app.import('vendor/sinon-qunit/lib/sinon-qunit.js', { type: 'test' });
}
```
